...
export class ListComponent {
  ...
  constructor(private apiService: ApiService,private toastService:ToastrService){}
    ...
    addBtnUrl:string = "/gui-services/add";


   

  ngOnInit(){
    this.breadCrumbItems = [{ label: 'GUI Services' }, { label: 'list', active: true }];

    this.apiService.get('gui-service').subscribe((statusdata) => {
  ...
  actionBtn(row:any) {
    Swal.fire({
      ...
      if (result.value) {
        ...
        this.apiService.get('gui-service/delete/'+row.items._id).subscribe(
          (data) => {
