...
export class ListComponent {
  ...
  constructor(private apiService: ApiService,private toastService:ToastrService){}
    ...
    addBtnUrl:string = "/train-services/add";


   

  ngOnInit(){
    this.breadCrumbItems = [{ label: 'Train Services' }, { label: 'list', active: true }];

    this.apiService.get('train-service').subscribe((statusdata) => {
  ...
  actionBtn(row:any) {
    Swal.fire({
      ...
      if (result.value) {
        ...
        this.apiService.get('train-service/delete/'+row.items._id).subscribe(
          (data) => {
