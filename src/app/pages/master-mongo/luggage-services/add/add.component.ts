...
export class AddComponent {
  ...
  constructor(
    ...
    init() {
      ...
      this.apiService.init('luggage-service/init').subscribe(
        (data) => {
    ...
    getById(id:string){
      this.apiService.get('luggage-service/'+id).subscribe(
        (data) => {
          if(data.length == 1){
  ...
  ngOnInit(): void {
    ...
    if(this._id){
     ...
     this.breadCrumbItems = [{ label: 'Luggage Service' }, { label: 'Update', active: true }];
     this.getById(this._id);
    } else {
     this.breadCrumbItems = [{ label: 'Luggage Service' }, { label: 'Add', active: true }];
    }
  }
  ...
  onSubmit() {
    ...
    if (this.parentForm.valid) {
      ...
      payload.id = this._id;
      this.apiService.postData('luggage-service',payload).subscribe(
        (data) => {
          ...
          this._router.navigate(['/luggage-services']);
        },
        (err) => {
          this.submitted = false;
