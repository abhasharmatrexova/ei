...
export class ListComponent {
  ...
  constructor(private apiService: ApiService,private toastService:ToastrService){}
    ...
    addBtnUrl:string = "/luggage-services/add";


   

  ngOnInit(){
    this.breadCrumbItems = [{ label: 'Luggage Services' }, { label: 'list', active: true }];

    this.apiService.get('luggage-service').subscribe((statusdata) => {
  ...
  actionBtn(row:any) {
    Swal.fire({
      ...
      if (result.value) {
        ...
        this.apiService.get('luggage-service/delete/'+row.items._id).subscribe(
          (data) => {
