...
export class ListComponent {
  ...
  constructor(private apiService: ApiService,private toastService:ToastrService){}
    ...
    theadData = [
      ...
      {
        text: 'Ferry type',
        value: 'ferry_type.name',
      },
      {
        text: 'Ferry class',
        value: 'ferry_class.name',
      },
      {
        text: 'From City',
        value: 'from_city.geo_location',
        type:'location',
        locationNested:true
    ...
    addBtnUrl:string = "/ferry-services/add";


   

  ngOnInit(){
    this.breadCrumbItems = [{ label: 'Ferry Services' }, { label: 'list', active: true }];

    this.apiService.get('ferry-service').subscribe((statusdata) => {
  ...
  actionBtn(row:any) {
    Swal.fire({
      ...
      if (result.value) {
        ...
        this.apiService.get('ferry-service/delete/'+row.items._id).subscribe(
          (data) => {
