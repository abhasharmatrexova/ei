...
export class ListComponent {
  ...
  constructor(private apiService: ApiService,private toastService:ToastrService){}
    ...
    theadData = [
      ...
      {
        text: 'Date From',
        value: 'applicable_period_from',
        type:'date'
      },
      {
        text: 'Date To',
        value: 'applicable_period_to',
        type:'date'
      },
    ...
    addBtnUrl:string = "/ent-services/add";


   

  ngOnInit(){
    this.breadCrumbItems = [{ label: 'ENT Services' }, { label: 'list', active: true }];

    this.apiService.get('ent-service').subscribe((statusdata) => {
  ...
  actionBtn(row:any) {
    Swal.fire({
      ...
      if (result.value) {
        ...
        this.apiService.get('ent-service/delete/'+row.items._id).subscribe(
          (data) => {
