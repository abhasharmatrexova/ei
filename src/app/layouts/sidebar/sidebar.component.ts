...
export class SidebarComponent {
  ...
  constructor(private http: HttpClient,
    ...
    private keycloak: KeycloakService,private apiService: ApiService, private route:ActivatedRoute,private router:Router,private sanitizer: DomSanitizer,@Inject(DOCUMENT) private dom:any,private meta: Meta,private title: Title) { }

  ngOnInit(): void {
    //*****************/
    this.sharedDataService.headerFunctionTriggered.subscribe(() => {
  ...
  getSideMenuData() {
    this.apiService.sideMenuData().subscribe(
      (data) => {
        const menuData = data;
        this.role = localStorage.getItem('role');      
        let matchingValue;
        // console.log("menuData",menuData,this.role);

        // Iterate through the object to find the matching value
        for (const key in menuData) {
          if (key === this.role) {
            matchingValue = menuData[key];
            break;
          }
        }

        if (matchingValue) {
          this.menuItems = matchingValue;
        } else {
